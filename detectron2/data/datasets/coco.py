...
"""
    ...
    for (img_dict, anno_dict_list) in imgs_anns:
        ...
        for anno in anno_dict_list:
            ...
            # amodal datasets 
            v_mask = anno.get("visible_mask", None)
            if v_mask:  # either list[list[float]] or dict(RLE)
                if not isinstance(v_mask, dict):
                    # filter out invalid polygons (< 3 points)
                    v_mask = [poly for poly in v_mask if len(poly) % 2 == 0 and len(poly) >= 6]
                    if len(v_mask) == 0:
                        num_instances_without_valid_v_mask += 1
                        continue  # ignore this instance
